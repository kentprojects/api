...
abstract class Date
{
	...
	public static function format($style, $timestamp = null)
	{
		if ($style === static::RELATIVE)
			return static::relative($timestamp);
	...
	public static function relative($timestamp)
	{
		...
		elseif($diff < $min) return $english($diff,'second');
		
		// If it's a few minutes ago.
		elseif ($diff < $hour) return $english(round($diff/$min),'minute');
		
		// It's a few hours ago.
		elseif ($diff < $day) return $english(round($diff/$hour),'hour');
		
		// It's a few days ago.
		elseif ($diff < $week) return $english(round($diff/$day),'day');
